...
class CalenderWidgetLargeConfigurationActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val views = RemoteViews(this.packageName, R.layout.calender_widget_large)

        formula1.setOnClickListener {
            SharedPreferences.write(appWidgetId.toString(), "f1")
            val intent: Intent = Intent(this, CalenderWidgetLarge::class.java)
            intent.setAction(AppWidgetManager.ACTION_APPWIDGET_UPDATE)
            val ids: IntArray? = AppWidgetManager.getInstance(this).getAppWidgetIds(ComponentName(this, CalenderWidgetLarge::class.java))
